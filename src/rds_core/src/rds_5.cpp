...
#include "rds/my_rvo.hpp"
#include "rds/distance_minimizer.hpp"
#include <RVO2/Agent.h> // from RVO2
#include <cmath>
#include <iostream>
...
namespace Geometry2D
{
			...
			Vec2* v_corrected_p_ref)
	{
		if (!ORCA_implementation)
		{
			...
			//std::cerr << "Reference point p_ref = (" << p_ref.x << ", " << p_ref.y << ")" << std::endl;
			...
			//std::cout << "v_nominal_p_ref.x = " << v_nominal_p_ref.x << ", v_corrected_p_ref.x = " << v_corrected_p_ref->x << std::endl;
